	...
	devtools(
		(set, get) => ({
			...
			fetchAll: async (env?: string) => {
				...
				const { process } = mockFetchAll;
				const { States = [], Roles = [], Companies = [] } = process.Globals;
				set(
					{ States, Roles, processes: [process], Companies, globalLoading: false },
					false,
					"fetchAll"
				);
				const activeProcessName = process.ProcessName;
